  ...
  group('$DirectorySettings', () {
    group('serializes', () {
      test('when empty throws an error', () {
        expect(() => DirectorySettings.fromJson({}), throwsA(isA<Exception>()));
      });

      test('successfully parses when path is only included', () {
        final settings = DirectorySettings.fromJson({'path': 'lib'});
    ...
    group('#resolveFileName', () {
      test('returns default file name', () {
        final settings = DirectorySettings(dirPath: 'lib');
      ...
      test('returns directory name', () {
        final settings = DirectorySettings(dirPath: 'lib/foo');
    ...
    test('#changePath changes only path', () {
      final settings = DirectorySettings(
        ...
        exports: [ExportSettings(export: 'foo')],
        include: [ExportSettings(export: 'bar')],
        exclude: ['baz'],
      );
